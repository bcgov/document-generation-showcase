...
def runStageDeploy(String stageEnv, String projectEnv, String hostEnv, String pathEnv) {
  ...
  openshift.withCluster() {
    openshift.withProject(projectEnv) {
      ...
      if(!(openshift.selector('cm', "dgrsc-frontend-config").exists() &&
      openshift.selector('cm', "dgrsc-server-config").exists() &&
      openshift.selector('cm', "dgrsc-services-config").exists() &&
      openshift.selector('secret', "app-keycloak-secret").exists() &&
      openshift.selector('secret', "cdogs-service-secret").exists())) {
